...
module.exports = {
  ...
  getProduct: (req, res) => {
    if(req.query.id) {
      let getQuery = `SELECT id_product as id, id_product, product_name, product_price, image_product, db_parshare.categories.category, product_quantity, db_parshare.products.active FROM products INNER JOIN db_parshare.categories ON db_parshare.products.id_category = db_parshare.categories.id_category WHERE id_product = ${req.query.id}`
      db.query(getQuery, (err, result) => {
        if(err){
          return res.status(500).send({
            success: false,
